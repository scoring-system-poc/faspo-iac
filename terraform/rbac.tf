...
resource "azurerm_role_assignment" "online-data-service-gha-acr-rbac" {
  principal_id         = azuread_service_principal.online-data-service-gha-sp.object_id
  scope                = azurerm_container_registry.acr.id
  role_definition_name = "AcrPush"

  depends_on = [
    azuread_service_principal.online-data-service-gha-sp,
    azurerm_container_registry.acr
  ]
}
...
resource "azurerm_role_assignment" "request-handler-gha-acr-rbac" {
  principal_id         = azuread_service_principal.request-handler-gha-sp.object_id
  scope                = azurerm_container_registry.acr.id
  role_definition_name = "AcrPush"

  depends_on = [
    azuread_service_principal.request-handler-gha-sp,
    azurerm_container_registry.acr
  ]
}
